...
const char* host = "grip-sense.onrender.com";
const int port = 443;
const int ENA = 25;
const int IN1 = 26;
const int IN2 = 27;
...
void socketEvent(socketIOmessageType_t type, uint8_t * payload, size_t length) {
  ...
  switch(type) {
    ...
    case sIOtype_CONNECT:
      ...
      // now register
      socket.sendEVENT("[\"register\",\"esp32\"]");
      break;
...
void setup() {
  ...
  // SOCKET CONNECT
  socket.beginSSL(host, port, "/socket.io/?EIO=4");
...
void loop() {
  ...
  if (millis() - windowStart >= windowTime) {
    ...
    if (consecutiveDanger >= 6) {
      riskLevel = "CRITICAL";
    } else if (consecutiveDanger >= 4) {
      riskLevel = "RISK";
    }

    // Create JSON string
    String json = "{";
    json += "\"fsr1\":" + String(norm1, 1) + ",";
    json += "\"fsr2\":" + String(norm2, 1) + ",";
    json += "\"motionStatus\":\"" + currentStatus + "\",";
    json += "\"avgRate\":" + String(avgRate, 2) + ",";
    json += "\"consecutiveDanger\":" + String(consecutiveDanger) + ",";
    json += "\"consecutiveMedium\":" + String(consecutiveMedium) + ",";
    json += "\"consecutiveNormal\":" + String(consecutiveNormal) + ",";
    json += "\"fsrReduction\":" + String(fsrSpeedReduction) + ",";
